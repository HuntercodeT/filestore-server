...
func SigninHandler(w http.ResponseWriter, r *http.Request) {
	...
	// ExpiresAt := strconv.FormatInt(Expirestime, 10)
...
func UserInfoHandler(w http.ResponseWriter, r *http.Request) {
	...
	r.ParseForm()
	username := r.Form.Get("username")
	// expiresTime := r.Form.Get("expiresTime")
	// expiresAt, err := strconv.ParseInt(expiresTime, 10, 64)
	// if err != nil {
	// 	fmt.Println("过期时间出错")
	...
	// // fmt.Println(token)
	// // ExpiresTime := r.Form.Get("ExpiresAt")
	// if authHeader == "" {
	// 	resp := util.RespMsg{
	// 		Code: 2003,
	// 		Msg:  "请求头中auth为空",
...
func KeyExpired(expires int64) bool {
	if expires >= 1 {
		return time.Now().In(cstSh).After(time.Unix(expires, 0))
	}
...
func GenToken(username string) (int64, string, error) {
	...
	c := MyClaims{
		...
		jwt.StandardClaims{
			...
			IssuedAt:  time.Now().In(cstSh).Unix(),
		},
	}
